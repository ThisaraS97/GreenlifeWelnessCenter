  ...
  //$start_date = $_POST['startdate'] ?? null;
    ...
    if ($password !== $confirm) {
        ...
        if ($check->num_rows > 0) {
            ...
            // Plans removed — no duration/start_date stored
            $duration = '';
            $start_date = null;
            $hashed = password_hash($password, PASSWORD_DEFAULT);

            $stmt = $conn->prepare("INSERT INTO members (full_name, email, phone, dob, gender, password)
                                    VALUES (?, ?, ?, ?, ?, ?)");
...
<head>
  ...
  color: #509956;
    ...
    h4 {
  color: #509956;
  border-bottom: 2px solid #509956;
      padding-bottom: 10px;
    }
...
<div class="container">
  <h2>Join GreenLife Wellness Center</h2>
